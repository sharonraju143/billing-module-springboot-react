...
const TopResourseTypeBarChart = ({ top5ServiceDescriptions }) => {
  ...
  const uniqueServices = Array.from(new Set(top5ServiceDescriptions.map(item => item.serviceDescription)));

  const transformedData = top5ServiceDescriptions.map((item, index) => ({
    serviceDescription: item.serviceDescription,
    totalCost: parseFloat(item.totalCost.toFixed(2)), // Round to 2 decimal places
    color: index, // Assign index as a color identifier
  }));
      ...
      <div className="chart-pie">
        ...
        <div className="responsive-chart">
          <BarChart width={700} height={400} data={transformedData} layout="vertical">
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey="totalCost" fill={({ payload }) => payload.color} />
          </BarChart>
        </div>
      </div>
    </div>
